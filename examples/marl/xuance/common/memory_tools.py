...
from xuance.common import Dict, Optional, Union, discount_cumsum, space2shape
from xuance.common.segtree_tool import MinSegmentTree, SumSegmentTree


def create_memory(shape: Optional[Union[tuple, dict]], n_envs: int, n_size: int, dtype: type = np.float32):
    """
...
class Buffer(ABC):
    ...
    def __init__(self, observation_space: Space, action_space: Space, auxiliary_info_shape: Optional[dict]):
...
class EpisodeBuffer:
    ...
    def sample(self, lookup_step=None, idx=None) -> Dict[str, np.ndarray]:
        ...
        return {"obs": obs, "acts": action, "rews": reward, "done": done}
...
class DummyOnPolicyBuffer(Buffer):
    ...
    def __init__(
        ...
        super().__init__(observation_space, action_space, auxiliary_shape)
    ...
    def sample(self, indexes):
        ...
        samples_dict = {
            "obs": sample_batch(self.observations, (env_choices, step_choices)),
            "actions": sample_batch(self.actions, (env_choices, step_choices)),
            "returns": sample_batch(self.returns, (env_choices, step_choices)),
            "values": sample_batch(self.values, (env_choices, step_choices)),
        ...
        samples_dict.update({"advantages": adv_batch})
...
class DummyOnPolicyBuffer_Atari(DummyOnPolicyBuffer):
    ...
    def __init__(
        ...
        super().__init__(
            observation_space, action_space, auxiliary_shape, n_envs, horizon_size, use_gae, use_advnorm, gamma, gae_lam
        )
...
class DummyOffPolicyBuffer(Buffer):
    ...
    def sample(self, batch_size=None):
        ...
        samples_dict = {
            "obs": sample_batch(self.observations, (env_choices, step_choices)),
            "actions": sample_batch(self.actions, (env_choices, step_choices)),
            "obs_next": sample_batch(self.next_observations, (env_choices, step_choices)),
            "rewards": sample_batch(self.rewards, (env_choices, step_choices)),
            "terminals": sample_batch(self.terminals, (env_choices, step_choices)),
            "batch_size": bs,
        }
        return samples_dict
...
class RecurrentOffPolicyBuffer(Buffer):
    ...
    def __init__(
        ...
        super().__init__(observation_space, action_space, auxiliary_shape)
        self.n_envs, self.buffer_size, self.episode_length, self.batch_size = (
            n_envs,
            buffer_size,
            episode_length,
            batch_size,
...
class PerOffPolicyBuffer(Buffer):
    ...
    def sample(self, beta):
        ...
        samples_dict = {
            "obs": sample_batch(self.observations, (env_choices, step_choices.flatten())),
            "actions": sample_batch(self.actions, (env_choices, step_choices.flatten())),
            "obs_next": sample_batch(self.next_observations, (env_choices, step_choices.flatten())),
            "rewards": sample_batch(self.rewards, (env_choices, step_choices.flatten())),
            "terminals": sample_batch(self.terminals, (env_choices, step_choices.flatten())),
            "weights": weights,
...
class DummyOffPolicyBuffer_Atari(DummyOffPolicyBuffer):
    ...
    def __init__(
        ...
        super().__init__(observation_space, action_space, auxiliary_shape, n_envs, buffer_size, batch_size)
        self.observations = create_memory(space2shape(self.observation_space), self.n_envs, self.n_size, np.uint8)
...
class SequentialReplayBuffer(Buffer):
    ...
    def sample(self, seq_len: int):
        ...
        samples_dict = {  # (envs, seq, batch, ~)
            "obs": self.obs[envs, idxes].reshape(
                self.n_envs, seq_len, self.batch_size, *space2shape(self.observation_space)
            ),
            "acts": self.acts[envs, idxes].reshape(
                self.n_envs, seq_len, self.batch_size, *space2shape(self.action_space)
            ),
