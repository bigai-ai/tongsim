...
def get_configs(file_dir):
    ...
    with open(file_dir, encoding="utf-8") as f:
        try:
            ...
            raise AssertionError(file_dir + f" error: {exc}") from exc
    return config_dict
...
def get_arguments(method, env, env_id, config_path=None, parser_args=None, is_test=False):
    ...
    """
    main_path = os.getcwd()
    config_path_default = os.path.join(main_path, "examples", "marl", "example", "config")
    ...
    if isinstance(method, list):  # for different groups of MARL algorithms.
        if config_path is None:
            ...
            for i_agent, _agent in enumerate(method):
...
def get_runner(method, env, env_id, config_path=None, parser_args=None, tag=None, is_test=False):
    """
    ...
    dl_toolbox = (
        args[0].dl_toolbox if isinstance(args, list) else args.dl_toolbox
    )  # The choice of deep learning toolbox.
    ...
    if distributed_training:
        if rank == 0:
            print("Calculating device: Multi-GPU distributed training.")
    else:
    ...
    if isinstance(args, list):
        ...
        for i_alg in range(len(method)):
            ...
            if ("model_dir" in args[i_alg].__dict__) and ("log_dir" in args[i_alg].__dict__):
                args[i_alg].model_dir = os.path.join(
                    os.getcwd(), args[i_alg].model_dir + notation + args[i_alg].env_id + "/", f"side_{tag}{i_alg}/"
                )
                args[i_alg].log_dir = args[i_alg].log_dir + notation + args[i_alg].env_id + f"/side_{tag}{i_alg}/"
            else:
        ...
        if ("model_dir" in args.__dict__) and ("log_dir" in args.__dict__):
            ...
            if config_path is not None:
                ...
                raise AttributeError(
                    f"The environment named '{args.env}' is currently not supported for {args.method}."
                )
            else:
                if rank == 0:
                    print("Failed to load arguments for the implementation!")
