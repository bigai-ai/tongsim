...
from xuance.common import Dict, List, Optional, create_memory, space2shape
...
class MARL_OnPolicyBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(agent_keys, state_space, obs_space, act_space, n_envs, buffer_size)
        ...
        self.reward_space = dict.fromkeys(self.agent_keys, ())
        self.returns = dict.fromkeys(self.agent_keys, ())
        self.values = dict.fromkeys(self.agent_keys, ())
        self.log_pi_old = dict.fromkeys(self.agent_keys, ())
        self.advantages = dict.fromkeys(self.agent_keys, ())
        self.terminal_space = dict.fromkeys(self.agent_keys, ())
        self.agent_mask_space = dict.fromkeys(self.agent_keys, ())
        self.clear()
    ...
    def clear(self):
        ...
        if self.store_global_state:
            self.data.update({"state": create_memory(space2shape(self.state_space), self.n_envs, self.n_size)})
    ...
    def finish_path(self, i_env: Optional[int] = None, value_next: Optional[dict] = None, value_normalizer=None):
        """
        ...
        for key in self.agent_keys:
            ...
            if self.use_gae:
                for t in reversed(range(step_nums)):
                    if use_value_norm:
                        vs_t, vs_next = (
                            value_normalizer[key_vn].denormalize(vs[t]),
                            value_normalizer[key_vn].denormalize(vs[t + 1]),
                        )
                    else:
                ...
                advantages = (
                    returns - value_normalizer[key_vn].denormalize(vs[:-1]) if use_value_norm else returns - vs[:-1]
                )
            else:
...
class MARL_OnPolicyBuffer_RNN(MARL_OnPolicyBuffer):
    ...
    def clear(self):
        self.data = {
            "obs": {
                k: np.zeros((self.buffer_size, self.max_eps_len) + self.obs_shape[k], np.float32)
                for k in self.agent_keys
            },
            "actions": {
                k: np.zeros((self.buffer_size, self.max_eps_len) + self.act_shape[k], np.float32)
                for k in self.agent_keys
            },
        ...
        if self.use_actions_mask:
            self.data.update(
                {
                    "avail_actions": {
                        k: np.zeros((self.buffer_size, self.max_eps_len) + self.avail_actions_shape[k], dtype=np.bool_)
                        for k in self.agent_keys
                    }
                }
            )
    ...
    def clear_episodes(self):
        self.episode_data = {
            "obs": {
                k: np.zeros((self.n_envs, self.max_eps_len) + self.obs_shape[k], np.float32) for k in self.agent_keys
            },  # n_envs,max_eps_len,obs_shape
            "actions": {
                k: np.zeros((self.n_envs, self.max_eps_len) + self.act_shape[k], np.float32) for k in self.agent_keys
            },
        ...
        if self.use_actions_mask:
            self.episode_data.update(
                {
                    "avail_actions": {
                        k: np.zeros((self.n_envs, self.max_eps_len) + self.avail_actions_shape[k], dtype=np.bool_)
                        for k in self.agent_keys
                    }
                }
            )
    ...
    def store_episodes(self, i_env):
        ...
        print(f"store episode data, current buffer size is {self.size}")
...
class MeanField_OnPolicyBuffer(MARL_OnPolicyBuffer):
    ...
    def clear(self):
        super().clear()
        self.data["actions_mean"] = {
            k: create_memory(space2shape(self.prob_space), self.n_envs, self.n_size) for k in self.agent_keys
        }
...
class MeanField_OnPolicyBuffer_RNN(MARL_OnPolicyBuffer_RNN):
    ...
    def clear(self):
        super().clear()
        self.data["actions_mean"] = {
            k: np.zeros((self.buffer_size, self.max_eps_len) + self.prob_space, dtype=np.float32)
            for k in self.agent_keys
        }

    def clear_episodes(self):
        super().clear_episodes()
        self.episode_data["actions_mean"] = {
            k: np.zeros((self.n_envs, self.max_eps_len) + self.prob_space, dtype=np.float32) for k in self.agent_keys
        }
    ...
    def clear(self):
        ...
        self.data: {'obs': {'agent_0': shape=[50, 200, 18],
                    ...
                    'actions': {'agent_0': shape=[50, 200, 5],
                     ...
                     ...}
        """
        reward_space = dict.fromkeys(self.agent_keys, ())
        terminal_space = dict.fromkeys(self.agent_keys, ())
        agent_mask_space = dict.fromkeys(self.agent_keys, ())
...
class MARL_OffPolicyBuffer_RNN(MARL_OffPolicyBuffer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.obs_shape = {k: space2shape(obs_space[k]) for k in agent_keys}
        self.act_shape = {k: space2shape(act_space[k]) for k in agent_keys}
        super().__init__(agent_keys, state_space, obs_space, act_space, n_envs, buffer_size, batch_size, **kwargs)
        self.episode_data = {}
        self.clear_episodes()
    ...
    def clear(self):
        ...
        self.data = {
            "obs": {
                k: np.zeros((self.buffer_size, self.max_eps_len + 1) + self.obs_shape[k], dtype=np.float32)
                for k in self.agent_keys
            },
            "actions": {
                k: np.zeros((self.buffer_size, self.max_eps_len) + self.act_shape[k], dtype=np.float32)
                for k in self.agent_keys
            },
            ...
            self.data.update(
                {
                    "avail_actions": {
                        k: np.zeros(
                            (self.buffer_size, self.max_eps_len + 1) + self.avail_actions_shape[k], dtype=np.bool_
                        )
                        for k in self.agent_keys
                    }
                }
    ...
    def clear_episodes(self):
        ...
        self.episode_data = {
            "obs": {
                k: np.zeros((self.n_envs, self.max_eps_len + 1) + self.obs_shape[k], dtype=np.float32)
                for k in self.agent_keys
            },
            "actions": {
                k: np.zeros((self.n_envs, self.max_eps_len) + self.act_shape[k], dtype=np.float32)
                for k in self.agent_keys
            },
        ...
        if self.use_actions_mask:
            self.episode_data.update(
                {
                    "avail_actions": {
                        k: np.zeros((self.n_envs, self.max_eps_len + 1) + self.avail_actions_shape[k], dtype=np.bool_)
                        for k in self.agent_keys
                    }
                }
            )
...
class MeanField_OffPolicyBuffer_RNN(MARL_OffPolicyBuffer_RNN):
    ...
    def clear(self):
        super().clear()
        self.data["actions_mean"] = {
            k: np.zeros((self.buffer_size, self.max_eps_len + 1) + self.prob_space, dtype=np.float32)
            for k in self.agent_keys
        }
    ...
    def clear_episodes(self):
        super().clear_episodes()
        self.episode_data["actions_mean"] = {
            k: np.zeros((self.n_envs, self.max_eps_len + 1) + self.prob_space, dtype=np.float32)
            for k in self.agent_keys
        }
