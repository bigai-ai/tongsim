...
from xuance.common import List, Optional, get_configs
...
def build_search_space(trial: optuna.trial, hyperparameters: List[Hyperparameter]) -> dict:
    ...
    for param in hyperparameters:
        if isinstance(param.distribution, list) or param.type == "categorical":
            ...
            if param.type == "float":
                if param.log:
                    search_space[param.name] = trial.suggest_loguniform(
                        param.name, low=param.distribution[0], high=param.distribution[1]
                    )
                else:
                    search_space[param.name] = trial.suggest_uniform(
                        param.name, low=param.distribution[0], high=param.distribution[1]
                    )
            elif param.type == "int":
...
class HyperParameterTuner:
    ...
    def __init__(
        self, method: str, config_path: str, running_steps: Optional[int] = None, test_episodes: Optional[int] = None
    ):
        """
...
class MultiObjectiveTuner(HyperParameterTuner):
    ...
    def objective(
        self, trial: optuna.trial, selected_hyperparameters: List[Hyperparameter], selected_objectives: List[str] = None
    ) -> float:
        """
        Define the objective function for Optuna optimization.
    ...
    def tune(
        ...
        selected_objectives: List[str] = None,
    ) -> optuna.study.Study:
        """
        Start the hyperparameter tuning process.
