...
from xuance.common import Sequence, Union
...
class RunningMeanStd:
    ...
    def __init__(self, shape: Union[Sequence[int], dict], epsilon=1e-4, comm=None, use_mpi=False):
        """
        ...
        if isinstance(shape, dict):
            ...
            self.count = dict.fromkeys(shape.keys(), epsilon)
        else:
        ...
        if isinstance(batch_mean, dict):
            for key in self.shape:
                ...
                M2 = (
                    m_a
                    + m_b
                    + np.square(delta) * self.count[key] * batch_count[key] / (self.count[key] + batch_count[key])
                )
