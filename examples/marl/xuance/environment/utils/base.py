...
from xuance.common import Any, Dict, List, Optional, Tuple, Union
...
class RawMultiAgentEnv(ABC):
    ...
    def get_groups_info(self) -> Dict[str, Any]:
        return {
            ...
            "observation_space_groups": [
                {k: self.observation_space[k] for i, k in enumerate(group)} for group in self.agent_groups
            ],
            "action_space_groups": [
                {k: self.action_space[k] for i, k in enumerate(group)} for group in self.agent_groups
            ],
    ...
    def agent_mask(self):
        ...
        return dict.fromkeys(self.agents, True)
    ...
    ) -> Tuple[MultiAgentDict, MultiAgentDict, MultiAgentDict, bool, MultiAgentDict]:
        """
        Steps through the environment with action.
