...
from xuance.environment.utils import (
...
def make_envs(config: Namespace):
    ...
    def _thunk(env_seed: int = None):
        ...
        if config.env_name in REGISTRY_MULTI_AGENT_ENV.keys():
            if getattr(config, "UE", False):
                return REGISTRY_MULTI_AGENT_ENV[config.env_name](config)
            else:
