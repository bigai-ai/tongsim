...
from xuance.common import Callable, Dict, List, Optional, Sequence, Union
from xuance.torch import DistributedDataParallel, Module, ModuleDict, Tensor
from xuance.torch.policies import ActorNet, BasicQhead, CriticNet, QMIX_FF_mixer, VDN_mixer
...
class BasicQnetwork(Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for key in self.model_keys:
            ...
            self.eval_Qhead[key] = BasicQhead(
                self.dim_input_Q[key], self.n_actions[key], hidden_size, normalize, initialize, activation, device
            )
            ...
            for key in self.model_keys:
                if self.representation[key]._get_name() != "Basic_Identical":
                    self.representation[key] = DistributedDataParallel(
                        module=self.representation[key], device_ids=[self.rank]
                    )
    ...
    def forward(
        ...
        for key in agent_list:
            ...
            if self.use_parameter_sharing:
                q_inputs = torch.concat([outputs["state"], agent_ids], dim=-1)  # CONCATENATE THE AGENT ID
            else:
                q_inputs = outputs["state"]

            evalQ[key] = self.eval_Qhead[key](q_inputs)  # 512,66->512,5
        ...
        return rnn_hidden_new, argmax_action, evalQ  # RETURN THE ACTION AND THE LOGITS
...
class MixingQnetwork(BasicQnetwork):
    ...
    def parameters_model(self):
        parameters_model = (
            list(self.eval_Qtot.parameters())
            + list(self.representation.parameters())
            + list(self.eval_Qhead.parameters())
        )
        return parameters_model
    ...
    def Q_tot(self, individual_values: Dict[str, Tensor], states: Optional[Tensor] = None):
        ...
        if self.use_parameter_sharing:
            ...
            """
            individual_inputs = individual_values[self.model_keys[0]].reshape(
                [-1, self.n_agents, 1]
            )  # DIVIDE BY N_AGENTS
        else:
            """
            ...
            """
            individual_inputs = torch.concat([individual_values[k] for k in self.model_keys], dim=-1).reshape(
                [-1, self.n_agents, 1]
            )
        evalQ_tot = self.eval_Qtot(
            individual_inputs, states
        )  # input is the individual q values of each agent and global state but now state is none
        return evalQ_tot
...
class Weighted_MixingQnetwork(MixingQnetwork):
        ...
        if self.distributed_training:
            for key in self.model_keys:
                self.eval_Qhead_centralized[key] = DistributedDataParallel(
                    module=self.eval_Qhead_centralized[key], device_ids=[self.rank]
                )[key]
    ...
    def parameters_model(self):
        parameters_model = (
            list(self.eval_Qtot.parameters())
            + list(self.ff_mixer.parameters())
            + list(self.representation.parameters())
            + list(self.eval_Qhead.parameters())
            + list(self.eval_Qhead_centralized.parameters())
        )
        return parameters_model
    ...
    def q_centralized(
        ...
        """
        rnn_hidden_new, _argmax_action, evalQ_cent = {}, {}, {}
        agent_list = self.model_keys if agent_key is None else [agent_key]
...
class Qtran_MixingQnetwork(BasicQnetwork):
    ...
    def parameters_model(self):
        parameters_model = (
            list(self.qtran_net.parameters())
            + list(self.q_tot.parameters())
            + list(self.representation.parameters())
            + list(self.eval_Qhead.parameters())
        )
        return parameters_model
    ...
    def Q_tran(
        ...
        if self.use_parameter_sharing:
            ...
            if agent_mask is not None:
                if self.use_rnn:
                    agent_mask = (
                        agent_mask[key].reshape(batch_size, self.n_agents, seq_len, 1).repeat(1, 1, 1, dim_hidden_state)
                    )
                else:
            ...
            actions_onehot = torch.cat(
                [one_hot(actions[k].long(), self.n_actions_max).unsqueeze(1) for k in self.model_keys], dim=1
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if hidden_size_bias is not None:
            ...
            self.bias = BasicQhead(state_dim, 1, 0, hidden_size_bias, normalize, initialize, activation, device)
            self.target_bias = deepcopy(self.bias)

    @property
    def parameters_model(self):
        parameters_model = (
            list(self.representation.parameters()) + list(self.utility.parameters()) + list(self.payoffs.parameters())
        )
        if self.dcg_s:
...
class MFQnetwork(Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for key in self.model_keys:
            ...
            self.dim_input_Q[key] = (
                self.representation_info_shape[key]["state"][0] + kwargs["action_embedding_hidden_size"][-1]
            )
            self.n_actions[key] = self.action_space[key].n
            if self.use_parameter_sharing:
                self.dim_input_action_embedding[key] += self.n_agents
                self.dim_input_Q[key] += self.n_agents
            self.action_mean_embedding[key] = Basic_MLP(
                (self.dim_input_action_embedding[key],),
        ...
        if self.distributed_training:
            ...
            for key in self.model_keys:
                ...
                self.action_mean_embedding[key] = DistributedDataParallel(
                    module=self.action_mean_embedding[key], device_ids=[self.rank]
                )
                self.eval_Qhead[key] = DistributedDataParallel(module=self.eval_Qhead[key], device_ids=[self.rank])
    ...
    def parameters_model(self):
        parameters_model = {}
        for key in self.model_keys:
            parameters_model[key] = (
                list(self.representation[key].parameters())
                + list(self.action_mean_embedding[key].parameters())
                + list(self.eval_Qhead[key].parameters())
            )
        return parameters_model
    ...
    def get_mean_actions(self, actions: Dict[str, Tensor], agent_mask_tensor: Tensor, batch_size: int):
        if self.use_parameter_sharing:
...
class Independent_DDPG_Policy(Module):
            ...
            for key in self.model_keys:
                if self.actor_representation[key]._get_name() != "Basic_Identical":
                    self.actor_representation[key] = DistributedDataParallel(
                        module=self.actor_representation[key], device_ids=[self.rank]
                    )
                if self.critic_representation[key]._get_name() != "Basic_Identical":
                    self.critic_representation[key] = DistributedDataParallel(
                        module=self.critic_representation[key], device_ids=[self.rank]
                    )
    ...
    def forward(
        ...
        for key in agent_list:
            ...
            if self.use_parameter_sharing:
                actor_in = torch.concat([outputs["state"], agent_ids], dim=-1)  # 2,244  2,23
            else:
...
class MADDPG_Policy(Independent_DDPG_Policy):
    ...
    def Qtarget(
        ...
        critic_rep_in = torch.concat([joint_observation, joint_actions], dim=-1)  # 256,30
        if self.use_rnn:
        ...
        for key in agent_list:
            if self.use_parameter_sharing:
                if self.use_rnn:
                    ...
                    joint_rep_out = outputs[key]["state"].unsqueeze(1).expand(-1, self.n_agents, -1)
                    joint_rep_out = joint_rep_out.reshape(bs, -1)
                critic_in = torch.concat([joint_rep_out, agent_ids], dim=-1)  # 512，32
            else:
                if self.use_rnn:
                    joint_rep_out = outputs[key]["state"].reshape(bs, seq_len, -1)
                else:
...
class MATD3_Policy(MADDPG_Policy, Module):
        ...
        **kwargs,
        ...
        for key in self.model_keys:
            ...
            dim_actor_in, dim_actor_out, dim_critic_in = self._get_actor_critic_input(
                self.actor_representation[key].output_shapes["state"][0],
                dim_action,
                self.critic_A_representation[key].output_shapes["state"][0],
                n_agents,
            )
        ...
        if self.distributed_training:
            ...
            for key in self.model_keys:
                if self.actor_representation[key]._get_name() != "Basic_Identical":
                    self.actor_representation[key] = DistributedDataParallel(
                        self.actor_representation[key], device_ids=[self.rank]
                    )
                if self.critic_A_representation[key]._get_name() != "Basic_Identical":
                    self.critic_A_representation[key] = DistributedDataParallel(
                        self.critic_A_representation[key], device_ids=[self.rank]
                    )
                if self.critic_B_representation[key]._get_name() != "Basic_Identical":
                    self.critic_B_representation[key] = DistributedDataParallel(
                        self.critic_B_representation[key], device_ids=[self.rank]
                    )
                self.actor[key] = DistributedDataParallel(module=self.actor[key], device_ids=[self.rank])
    ...
    def parameters_critic(self):
        parameters_critic = {}
        for key in self.model_keys:
            parameters_critic[key] = (
                list(self.critic_A_representation[key].parameters())
                + list(self.critic_A[key].parameters())
                + list(self.critic_B_representation[key].parameters())
                + list(self.critic_B[key].parameters())
            )
        return parameters_critic
