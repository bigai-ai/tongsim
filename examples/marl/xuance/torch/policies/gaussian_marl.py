...
from .core import CriticNet, GaussianActorNet, GaussianActorNet_SAC
...
class MAAC_Policy(Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for key in self.model_keys:
            ...
            self.actor[key] = GaussianActorNet(
                dim_actor_in,
                dim_actor_out,
                actor_hidden_size,
                normalize,
    ...
    def parameters_model(self):
        parameters = (
            list(self.actor_representation.parameters())
            + list(self.actor.parameters())
            + list(self.critic_representation.parameters())
    ...
    def forward(
        ...
        agent_ids: Optional[Tensor] = None,
...
class Basic_ISAC_Policy(Module):
        ...
        **kwargs,
        ...
        for key in self.model_keys:
            ...
            dim_actor_in, dim_actor_out, dim_critic_in = self._get_actor_critic_input(
                self.actor_representation[key].output_shapes["state"][0],
                dim_action,
                self.critic_1_representation[key].output_shapes["state"][0],
                n_agents,
            )

            self.actor[key] = GaussianActorNet_SAC(
                dim_actor_in,
                dim_actor_out,
                actor_hidden_size,
                normalize,
        ...
        if self.distributed_training:
            ...
            for key in self.model_keys:
                ...
                if self.critic_1_representation[key]._get_name() != "Basic_Identical":
                    self.critic_1_representation[key] = DistributedDataParallel(
                        self.critic_1_representation[key], device_ids=[self.rank]
                    )
                if self.critic_2_representation[key]._get_name() != "Basic_Identical":
                    self.critic_2_representation[key] = DistributedDataParallel(
                        self.critic_2_representation[key], device_ids=[self.rank]
                    )
                self.actor[key] = DistributedDataParallel(module=self.actor[key], device_ids=[self.rank])
    ...
    def parameters_critic(self):
        ...
        for key in self.model_keys:
            parameters_critic[key] = (
                list(self.critic_1_representation[key].parameters())
                + list(self.critic_1[key].parameters())
                + list(self.critic_2_representation[key].parameters())
                + list(self.critic_2[key].parameters())
            )
    ...
    def Qtarget(
        ...
        for key in agent_list:
            if self.use_rnn:
                outputs_critic_1 = self.target_critic_1_representation[key](
                    next_observation[key], *rnn_hidden_critic_1[key]
                )
                outputs_critic_2 = self.target_critic_2_representation[key](
                    next_observation[key], *rnn_hidden_critic_2[key]
                )
                rnn_hidden_critic_new_1.update({key: (outputs_critic_1["rnn_hidden"], outputs_critic_1["rnn_cell"])})
                rnn_hidden_critic_new_2.update({key: (outputs_critic_2["rnn_hidden"], outputs_critic_2["rnn_cell"])})
            else:
...
class MASAC_Policy(Basic_ISAC_Policy):
    ...
    def Qpolicy(
        ...
        if self.use_rnn:
            outputs_critic_1 = {
                k: self.critic_1_representation[k](critic_rep_in, *rnn_hidden_critic_1[k]) for k in agent_list
            }
            outputs_critic_2 = {
                k: self.critic_2_representation[k](critic_rep_in, *rnn_hidden_critic_2[k]) for k in agent_list
            }
            rnn_hidden_critic_new_1.update(
                {k: (outputs_critic_1[k]["rnn_hidden"], outputs_critic_1[k]["rnn_cell"]) for k in agent_list}
            )
            rnn_hidden_critic_new_2.update(
                {k: (outputs_critic_2[k]["rnn_hidden"], outputs_critic_2[k]["rnn_cell"]) for k in agent_list}
            )
        else:
        ...
        for key in agent_list:
            if self.use_parameter_sharing:
                if self.use_rnn:
                    ...
                    joint_rep_out_1 = (
                        outputs_critic_1[key]["state"].unsqueeze(1).expand(-1, self.n_agents, -1).reshape(bs, -1)
                    )
                    joint_rep_out_2 = (
                        outputs_critic_2[key]["state"].unsqueeze(1).expand(-1, self.n_agents, -1).reshape(bs, -1)
                    )
    ...
    def Qtarget(
        ...
        if self.use_rnn:
            outputs_critic_1 = {
                k: self.target_critic_1_representation[k](critic_rep_in, *rnn_hidden_critic_1[k]) for k in agent_list
            }
            outputs_critic_2 = {
                k: self.target_critic_2_representation[k](critic_rep_in, *rnn_hidden_critic_2[k]) for k in agent_list
            }
            rnn_hidden_critic_new_1.update(
                {k: (outputs_critic_1[k]["rnn_hidden"], outputs_critic_1[k]["rnn_cell"]) for k in agent_list}
            )
