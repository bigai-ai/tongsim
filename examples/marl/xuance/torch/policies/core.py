...
from xuance.common import Callable, Dict, Optional, Sequence, Union
...
class CategoricalActorNet_SAC(CategoricalActorNet):
    ...
    def __init__(
        ...
        super().__init__(state_dim, action_dim, hidden_sizes, normalize, initialize, activation, device)
...
class VDN_mixer(nn.Module):
    ...
    def forward(self, values_n, states=None):
        return values_n.sum(dim=1)  # 256,1
...
class QMIX_mixer(nn.Module):
    ...
    def __init__(
        ...
        self.hyper_w_1 = nn.Sequential(
            nn.Linear(self.dim_state, self.dim_hypernet_hidden),
            nn.ReLU(),
        ...
        self.hyper_b_2 = nn.Sequential(
            nn.Linear(self.dim_state, self.dim_hypernet_hidden), nn.ReLU(), nn.Linear(self.dim_hypernet_hidden, 1)
        ).to(device)
...
class QMIX_FF_mixer(nn.Module):
    ...
    def __init__(
        ...
        self.ff_net_bias = nn.Sequential(
            nn.Linear(self.dim_state, self.dim_hidden), nn.ReLU(), nn.Linear(self.dim_hidden, 1)
        ).to(self.device)
...
class QTRAN_base(nn.Module):
    ...
    def __init__(
        ...
        self.Q_jt = nn.Sequential(
            nn.Linear(self.dim_q_input, self.dim_hidden),
            nn.ReLU(),
            nn.Linear(self.dim_hidden, self.dim_hidden),
            nn.ReLU(),
            nn.Linear(self.dim_hidden, 1),
        ).to(device)
        self.V_jt = nn.Sequential(
            nn.Linear(self.dim_v_input, self.dim_hidden),
            nn.ReLU(),
            nn.Linear(self.dim_hidden, self.dim_hidden),
            nn.ReLU(),
            nn.Linear(self.dim_hidden, 1),
        ...
        self.action_encoding = nn.Sequential(
            nn.Linear(self.dim_ae_input, self.dim_ae_input), nn.ReLU(), nn.Linear(self.dim_ae_input, self.dim_ae_input)
        ).to(device)
...
class QTRAN_alt(nn.Module):
    ...
    def __init__(
        ...
        self.Q_jt = nn.Sequential(
            ...
            nn.Linear(self.dim_hidden, self.dim_hidden),
            nn.ReLU(),
            nn.Linear(self.dim_hidden, self.n_actions_max),
        ).to(device)
        self.V_jt = nn.Sequential(
            nn.Linear(self.dim_v_input, self.dim_hidden),
            nn.ReLU(),
