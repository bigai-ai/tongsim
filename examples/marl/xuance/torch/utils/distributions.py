...
import numpy as np
import torch
from torch.distributions import Bernoulli, Categorical, Normal, constraints
from torch.distributions.utils import broadcast_all
from torch.nn.functional import one_hot, softmax, softplus

from xuance.common import Callable
from xuance.torch import Tensor
from xuance.torch.utils.operations import sym_exp, sym_log
...
class CategoricalDistribution(Distribution):
    ...
    def kl_divergence(self, other: Distribution):
        assert isinstance(other, CategoricalDistribution), (
            "KL Divergence should be measured by two same distribution with the same type"
        )
...
class ActivatedDiagGaussianDistribution(DiagGaussianDistribution):
    ...
    def activated_rsample_and_logprob(self):
        ...
        correction = -2.0 * (
            torch.log(Tensor([2.0])).to(self.device) - act_pre_activated - softplus(-2.0 * act_pre_activated)
        )
        log_prob += correction
...
class SymLogDistribution:
    def __init__(self, mode: Tensor, dims: int, dist: str = "mse", agg: str = "sum", tol: float = 1e-8):
        self._mode = mode
...
class TwoHotEncodingDistribution:
    ...
    def log_prob(self, x: Tensor) -> Tensor:
        ...
        target = (
            one_hot(below, len(self.bins)) * weight_below[..., None]
            + one_hot(above, len(self.bins)) * weight_above[..., None]
        ).squeeze(-2)
