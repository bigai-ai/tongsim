...
class Learner(ABC):
    def __init__(self, config: Namespace, policy: torch.nn.Module, callback):
    ...
    def load_model(self, path, model=None):
        ...
        self.policy.load_state_dict(
            torch.load(str(model_path), map_location={f"cuda:{i}": self.device for i in range(MAX_GPUs)})
        )
...
class LearnerMAS(ABC):
    def __init__(
        self, config: Namespace, model_keys: List[str], agent_keys: List[str], policy: torch.nn.Module, callback
    ):
        self.value_normalizer = None
        self.config = config
    ...
    def build_training_data(
        ...
        if use_parameter_sharing:
            ...
            if self.n_agents == 1:
                ...
                rewards_tensor = Tensor(np.stack(itemgetter(*self.agent_keys)(sample["rewards"]), axis=1)).to(
                    self.device
                )
                ter_tensor = (
                    Tensor(np.stack(itemgetter(*self.agent_keys)(sample["terminals"]), axis=1)).float().to(self.device)
                )
                msk_tensor = (
                    Tensor(np.stack(itemgetter(*self.agent_keys)(sample["agent_mask"]), axis=1)).float().to(self.device)
                )
            if self.use_rnn:
                ...
                IDs = (
                    ...
                    .unsqueeze(1)
                    .unsqueeze(0)
                    .expand(batch_size, -1, seq_length + 1, -1)
                    .reshape(bs, seq_length + 1, self.n_agents)
                    .to(self.device)
                )
            else:
                ...
                obs_next = {
                    k: Tensor(np.stack(itemgetter(*self.agent_keys)(sample["obs_next"]), axis=1))
                    .to(self.device)
                    .reshape(bs, -1)
                }
                IDs = (
            ...
            if use_actions_mask:
                ...
                if not self.use_rnn:
                    avail_actions_next = {
                        k: Tensor(sample["avail_actions_next"][k]).float().to(self.device) for k in self.model_keys
                    }
    ...
    def load_model(self, path, model=None, filter_prefix=None):
        ...
        if model is not None:
            ...
            if not os.path.isdir(candidate_path):
                raise RuntimeError(f"模型目录不存在: {candidate_path}")
            path = candidate_path
        else:
            all_names = os.listdir(path)

            dir_names = [d for d in all_names if os.path.isdir(os.path.join(path, d))]

            seed_dirs = [d for d in dir_names if "seed_" in d]

            if not seed_dirs:
                raise RuntimeError(f"未找到包含 seed_ 的模型目录: {path}")

            if filter_prefix:
                seed_dirs = [d for d in seed_dirs if any(d.startswith(p) or p in d for p in filter_prefix)]
                if not seed_dirs:
                    raise RuntimeError(f"过滤后无可用模型目录; filter_prefix={filter_prefix}")

            seed_dirs.sort()
            path = os.path.join(path, seed_dirs[-1])

        model_names = [f for f in os.listdir(path) if os.path.isfile(os.path.join(path, f))]

        if "obs_rms.npy" in model_names:
            model_names.remove("obs_rms.npy")

        if not model_names:
            raise RuntimeError(f"目录中没有模型文件: {path}")

        model_names.sort()
        model_path = os.path.join(path, model_names[-1])
        print(f"Loading model from '{model_path}'...")

        state_dict = torch.load(model_path, map_location={f"cuda:{i}": self.device for i in range(MAX_GPUs)})
