...
class IAC_Learner(LearnerMAS):
    def __init__(self, config: Namespace, model_keys: List[str], agent_keys: List[str], policy: nn.Module, callback):
        super().__init__(config, model_keys, agent_keys, policy, callback)
    ...
    def build_optimizer(self):
        self.optimizer = torch.optim.Adam(
            self.policy.parameters_model, lr=self.learning_rate, eps=1e-5, weight_decay=self.config.weight_decay
        )
        self.scheduler = torch.optim.lr_scheduler.LinearLR(
            self.optimizer, start_factor=1.0, end_factor=self.end_factor_lr_decay, total_iters=self.total_iters
        )
    ...
    def build_training_data(
        ...
        if use_parameter_sharing:
            ...
            obs_tensor = Tensor(np.stack(itemgetter(*self.agent_keys)(sample["obs"]), axis=1)).to(
                self.device
            )  # 2,3,200,115
            ...
            if self.use_rnn:
                obs = {k: obs_tensor.reshape(bs, seq_length, -1)}  # 6,200,115
                ...
                IDs = (
                    ...
                    .unsqueeze(1)
                    .unsqueeze(0)
                    .expand(batch_size, -1, seq_length, -1)
                    .reshape(bs, seq_length, self.n_agents)
                    .to(self.device)
                )
            else:
        ...
        if self.use_rnn:
            filled = Tensor(sample["filled"]).float().to(self.device)  # 2,200
    ...
    def update(self, sample):
        ...
        sample_Tensor = self.build_training_data(
            sample=sample, use_parameter_sharing=self.use_parameter_sharing, use_actions_mask=self.use_actions_mask
        )
        ...
        info = self.callback.on_update_start(
            self.iterations, method="update", policy=self.policy, sample_Tensor=sample_Tensor, bs=bs
        )
        ...
        for key in self.model_keys:
            ...
            info.update(
                self.callback.on_update_agent_wise(
                    self.iterations,
                    key,
                    info=info,
                    method="update",
    ...
    def update_rnn(self, sample):
        ...
        info = self.callback.on_update_start(
            self.iterations, method="update_rnn", policy=self.policy, sample_Tensor=sample_Tensor, bs_rnn=bs_rnn
        )
