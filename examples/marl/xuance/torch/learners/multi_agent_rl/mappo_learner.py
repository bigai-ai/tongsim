...
class MAPPO_Clip_Learner(IPPO_Learner):
    ...
    def update(self, sample):
        ...
        if self.use_parameter_sharing:
            ...
            if self.use_global_state:
                critic_input = {
                    key: state.reshape(batch_size, 1, -1).expand(batch_size, self.n_agents, -1).reshape(bs, -1)
                }
            else:
                critic_input = {
    ...
    def update_rnn(self, sample):
        ...
        if self.use_parameter_sharing:
            ...
            if self.use_global_state:
                critic_input = {
                    key: state.reshape(batch_size, 1, seq_len, -1)
                    .expand(-1, self.n_agents, -1, -1)
                    .reshape(bs_rnn, seq_len, -1)
                }
            else:
                joint_obs = (
                    obs[key]
                    .reshape(batch_size, self.n_agents, seq_len, -1)
                    .transpose(1, 2)
                    .reshape(batch_size, seq_len, -1)
                )
                joint_obs = (
                    joint_obs.unsqueeze(1).expand(-1, self.n_agents, -1, -1).reshape(bs_rnn, seq_len, -1)
                )  # 6，200，345
