...
from xuance.common import Callable, Optional, Sequence, Union
from xuance.torch import Module, Tensor
from xuance.torch.utils import ModuleType, mlp_block, nn, torch


# directly returns the original observation
class Basic_Identical(Module):
    def __init__(self, input_shape: Sequence[int], device: Optional[Union[str, int, torch.device]] = None, **kwargs):
        super().__init__()
    ...
    def _create_network(self):
        ...
        for h in self.hidden_sizes:
            mlp, input_shape = mlp_block(
                input_shape[0], h, self.normalize, self.activation, self.initialize, device=self.device
            )
            layers.extend(mlp)
