...
from xuance.common import EPS, Optional, RunningMeanStd, create_directory, get_time_string, space2shape
from xuance.torch import Module, REGISTRY_Learners, REGISTRY_Representation
from xuance.torch.utils import ActivationFunctions, NormalizeFunctions, init_distributed_mode, nn
...
class Agent:
    ...
    def save_model(self, model_name):
        ...
        if self.use_obsnorm:
            ...
            observation_stat = {"count": self.obs_rms.count, "mean": self.obs_rms.mean, "var": self.obs_rms.var}
        ...
        if self.use_obsnorm:
            if isinstance(self.observation_space, Dict):
                ...
                observations = np.clip(
                    (observations - self.obs_rms.mean) / (self.obs_rms.std + EPS),
                    -self.obsnorm_range,
                    self.obsnorm_range,
                )
            return observations
    ...
    def _build_representation(self, representation_key: str, input_space: Optional[Space], config: Namespace) -> Module:
        """
