...
from xuance.torch.agents.base import BaseCallback, MARLAgents
...
class OnPolicyMARLAgents(MARLAgents):
    ...
    def __init__(self, config: Namespace, envs, callback: Optional[BaseCallback] = None):
        super().__init__(config, envs, callback)
        self.on_policy = True
        continuous_action = config.continuous_action if hasattr(config, "continuous_action") else False
        self.continuous_control: bool = continuous_action
    ...
    def store_experience(
        self, obs_dict, avail_actions, actions_dict, log_pi_a, rewards_dict, values_dict, terminals_dict, info, **kwargs
    ):
        """
        ...
        if self.use_actions_mask:
            experience_data["avail_actions"] = {
                k: np.array([data[k] for data in avail_actions]) for k in self.agent_keys
            }
    ...
    def init_hidden_item(
        self, i_env: int, rnn_hidden_actor: Optional[dict] = None, rnn_hidden_critic: Optional[dict] = None
    ):
        """
    ...
    def action(
        ...
        **kwargs,
        ...
        rnn_hidden_actor_new, pi_dists = self.policy(
            observation=obs_input, agent_ids=agents_id, avail_actions=avail_actions_input, rnn_hidden=rnn_hidden_actor
        )
        if not test_mode:
            rnn_hidden_critic_new, values_out = self.policy.get_values(
                observation=obs_input, agent_ids=agents_id, rnn_hidden=rnn_hidden_critic
            )
        ...
        if self.use_parameter_sharing:
            ...
            actions_dict = [
                {k: actions_out[e, i].cpu().detach().numpy() for i, k in enumerate(self.agent_keys)}
            ...
            if self.continuous_control:
                actions_dict = [
                    {k: actions_sample[k].cpu().detach().numpy()[e].reshape([-1]) for k in self.agent_keys}
                    for e in range(n_env)
                ]
            else:
                actions_dict = [
                    {k: actions_sample[k].cpu().detach().numpy()[e].reshape([]) for k in self.agent_keys}
                    for e in range(n_env)
                ]
            if not test_mode:
        ...
        return {
            "rnn_hidden_actor": rnn_hidden_actor_new,
            "rnn_hidden_critic": rnn_hidden_critic_new,
            "actions": actions_dict,
            "log_pi": log_pi_a_dict,
            "values": values_dict,
        }

    def values_next(
        self, i_env: int, obs_dict: dict, state: Optional[np.ndarray] = None, rnn_hidden_critic: Optional[dict] = None
    ):
        """
        ...
        if self.use_parameter_sharing:
            ...
            if self.use_rnn:
                ...
                rnn_hidden_critic_i = {
                    key: self.policy.critic_representation[key].get_hidden_item(
            ...
            rnn_hidden_critic_new, values_out = self.policy.get_values(
                observation=obs_input, agent_ids=agents_id, rnn_hidden=rnn_hidden_critic_i
            )
            ...
            if self.use_rnn:
                rnn_hidden_critic_i = {
                    k: self.policy.critic_representation[k].get_hidden_item(
                        [
                            i_env,
                        ],
                        *rnn_hidden_critic[k],
    ...
    def train(self, n_steps):
        ...
        if self.use_rnn:
            with tqdm(total=n_steps) as process_bar:
                ...
                while step_last - step_start < n_steps_all:
                    ...
                    self.callback.on_train_epochs_end(
                        self.current_step,
                        policy=self.policy,
                        memory=self.memory,
                ...
                self.callback.on_train_step_end(
                    self.current_step, envs=self.envs, policy=self.policy, n_steps=n_steps, train_info=train_info
                )
            return train_info
        ...
        for _ in tqdm(range(n_steps)):
            policy_out = self.action(
                obs_dict=obs_dict, state=state, avail_actions_dict=avail_actions, test_mode=False
            )  # maybe there are some problems here
            ...
            reward_info = {
                "Train-Results/Step-Rewards/": {
                    "avg_all_envs": np.mean(
                        [np.mean(itemgetter(*self.agent_keys)(rewards_dict[i])) for i in range(self.n_envs)]
                    )
                }
            }
            self.log_infos(reward_info, self.current_step)

            self.callback.on_train_step(
                self.current_step,
                envs=self.envs,
                policy=self.policy,
                obs=obs_dict,
            ...
            if self.memory.full:
                for i in range(self.n_envs):
                    if all(terminated_dict[i].values()):
                        value_next = dict.fromkeys(self.agent_keys, 0.0)
                    else:
                    ...
                    self.memory.finish_path(
                        i_env=i, value_next=value_next, value_normalizer=self.learner.value_normalizer
                    )
            update_info = self.train_epochs(n_epochs=self.n_epochs)
            ...
            for i in range(self.n_envs):
                if all(terminated_dict[i].values()) or truncated[i]:
                    if all(terminated_dict[i].values()):
                        # print("An episode ends. Env ID: %d, all agent die" % i)
                        value_next = dict.fromkeys(self.agent_keys, 0.0)
                    else:
                        # print("An episode ends. Env ID: %d, truncated" % i)
                    ...
                    if self.use_wandb:
                        episode_info = {
                            f"Train-Results/Episode-Steps/rank_{self.rank}/env-{i}": info[i]["episode_step"],
                            f"Train-Results/Episode-Rewards/rank_{self.rank}/env-{i}": info[i]["episode_score"],
                        }
                    else:
                        episode_info = {
                            # f"Train-Results/Episode-Steps/rank_{self.rank}": {"env-%d" % i: info[i]["episode_step"]},
                            f"Train-Results/Episode-Rewards/rank_{self.rank}": {
                                f"env-{i}": np.mean(itemgetter(*self.agent_keys)(info[i]["episode_score"])),
                                "avg_all_envs": np.mean(
                                    [
                                        np.mean(itemgetter(*self.agent_keys)(info[j]["episode_score"]))
                                        for j in range(self.n_envs)
                                        if all(terminated_dict[j].values()) or truncated[j]
                                    ]
                                ),
                            }
                        }
    ...
    def run_episodes(self, env_fn=None, n_episodes: int = 1, test_mode: bool = False):
        ...
        if test_mode:
            ...
            if self.config.render_mode == "rgb_array" and self.render:
                print("Rendering...")
                images = envs.render(self.config.render_mode)  # Why here call the human render mode
                ...
                if all(terminated_dict[i].values()) or truncated[i]:
                    ...
                    if test_mode:
                        ...
                        if self.config.test_mode:
                            print(f"Episode: {_current_episode}, Score: {episode_score:.2f}")
                    else:
                        if all(terminated_dict[i].values()):
                            value_next = dict.fromkeys(self.agent_keys, 0.0)
                        else:
                            _, value_next = self.values_next(
                                i_env=i,
                                obs_dict=obs_dict[i],
                        ...
                        self.memory.finish_path(
                            i_env=i,
                            i_step=info[i]["episode_step"],
                            value_next=value_next,
                            value_normalizer=self.learner.value_normalizer,
                        )
                        if self.use_rnn:
                            rnn_hidden_actor, rnn_hidden_critic = self.init_hidden_item(
                                i, rnn_hidden_actor, rnn_hidden_critic
                            )
                        if self.use_wandb:
                            episode_info = {
                                f"Train-Results/Episode-Steps/env-{i}": info[i]["episode_step"],
                                f"Train-Results/Episode-Rewards/env-{i}": info[i]["episode_score"],
                            }
                        else:
                            episode_info = {
                                "Train-Results/Episode-Steps": {f"env-{i}": info[i]["episode_step"]},
                                "Train-Results/Episode-Rewards": {
                                    f"env-{i}" % i: np.mean(itemgetter(*self.agent_keys)(info[i]["episode_score"]))
                                },
                            }
