...
from xuance.common import MARL_OffPolicyBuffer, MARL_OffPolicyBuffer_RNN, Optional
...
def add_pre_squash_noise(
    actions_list: List[Dict[str, np.ndarray]], sigma: float, eps: float = 1e-6
) -> List[Dict[str, np.ndarray]]:
    noisy_actions_list = []
    for env_actions in actions_list:
        noisy_env_actions = {}
        for agent_id, action in env_actions.items():
            action = np.asarray(action, dtype=np.float32)

            clipped_action = np.clip(action, -1 + eps, 1 - eps)

            pre_squash_action = np.arctanh(clipped_action)

            noise = np.random.normal(loc=0.0, scale=sigma, size=action.shape)
            noisy_pre_squash_action = pre_squash_action + noise

            noisy_action = np.tanh(noisy_pre_squash_action)

            noisy_env_actions[agent_id] = noisy_action
        noisy_actions_list.append(noisy_env_actions)

    return noisy_actions_list
...
class OffPolicyMARLAgents(MARLAgents):
        ...
        **kwargs,
        ...
        if self.use_actions_mask:
            ...
            experience_data["avail_actions_next"] = {
                k: np.array([data[k] for data in avail_actions_next]) for k in self.agent_keys
            }
    ...
    def init_hidden_item(self, i_env: int, rnn_hidden: Optional[dict] = None):
        """
    ...
    def exploration(
        self, batch_size: int, pi_actions_dict: Union[List[dict], dict], avail_actions_dict: Optional[List[dict]] = None
    ):
        ...
        if self.e_greedy is not None:
            if np.random.rand() < self.e_greedy:
                if self.use_actions_mask:
                    explore_actions = [
                        {k: Categorical(Tensor(avail_actions_dict[e][k])).sample().numpy() for k in self.agent_keys}
                        for e in range(batch_size)
                    ]  # MAKE SURE THE ACTIION IS CORRECT AND VALID
                else:
                    explore_actions = [
                        {k: self.action_space[k].sample() for k in self.agent_keys} for _ in range(batch_size)
                    ]
            else:
    ...
    def action(
        ...
        **kwargs,
        ...
        hidden_state, actions, _ = self.policy(
            observation=obs_input, agent_ids=agents_id, avail_actions=avail_actions_input, rnn_hidden=rnn_hidden
        )
    ...
    def train(self, n_steps):
        ...
        for _ in tqdm(range(n_steps)):
            ...
            # here need to add the render menchanism for off-policy algorithms
            ...
            for i in range(self.n_envs):
                if all(terminated_dict[i].values()) or truncated[i]:
                    ...
                    if self.use_wandb:
                        ...
                        episode_info = {
                            f"Train-Results/Episode-Steps/rank_{self.rank}": {f"env-{i}": info[i]["episode_step"]},
    ...
    def run_episodes(self, env_fn=None, n_episodes: int = 1, test_mode: bool = False):
        ...
        if test_mode:
            # here are the entrance of inital rendering
        ...
        while _current_episode < n_episodes:
            policy_out = self.action(
                obs_dict=obs_dict, avail_actions_dict=avail_actions, rnn_hidden=rnn_hidden, test_mode=test_mode
            )
            rnn_hidden, actions_dict = policy_out["hidden_state"], policy_out["actions"]
            # actions_dict=[{'agent_0': 5, 'agent_1': 1}]  # for debug ,need to be deleted
            ...
            if test_mode:
                # here are the entrance of rendering
                if self.config.render_mode == "rgb_array" and self.render:
                    images = envs.render(self.config.render_mode)
                    for idx, img in enumerate(images):
                        videos[idx].append(img)
            ...
            for i in range(num_envs):
                if all(terminated_dict[i].values()) or truncated[i]:
                    ...
                    if self.use_rnn:
                        ...
                        if not test_mode:
                            terminal_data = {"obs": next_obs_dict[i], "episode_step": info[i]["episode_step"]}
                            if self.use_global_state:
    ...
    def train_epochs(self, n_epochs=1):
        ...
        """
        info_train = {}
        for _i_epoch in range(n_epochs):
            sample = self.memory.sample()
            # print("update epoch: ", i_epoch)
