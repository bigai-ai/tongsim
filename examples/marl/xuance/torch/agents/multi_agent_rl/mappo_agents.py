...
class MAPPO_Agents(IPPO_Agents):
    ...
    def _build_policy(self) -> Module:
        ...
        activation = ActivationFunctions[self.config.activation]  # relu
        ...
        if self.use_global_state:
            ...
            dim_obs_all = sum([sum(self.observation_space[k].shape) for k in self.agent_keys])  # 484
        space_critic_in = dict.fromkeys(self.agent_keys, (dim_obs_all,))
        C_representation = self._build_representation(self.config.representation, space_critic_in, self.config)
        # build policies
        policy_settings = {
            "action_space": self.action_space,
            "n_agents": self.n_agents,
            "representation_actor": A_representation,
            "representation_critic": C_representation,
            ...
            "rnn": self.config.rnn if self.use_rnn else None,
        }
    ...
    def _build_critic_inputs(self, batch_size: int, obs_batch: dict, state: Optional[np.ndarray]):
        ...
        critic_input_dict = dict.fromkeys(self.model_keys, critic_input)
        ...
        **kwargs,
        ...
        if not test_mode:
            ...
            rnn_hidden_critic_new, values_out = self.policy.get_values(
                observation=critic_input, agent_ids=agents_id, rnn_hidden=rnn_hidden_critic
            )
        ...
        if self.use_parameter_sharing:
            ...
            if self.continuous_control:
                ...
                actions_out = actions_sample.reshape(n_env, self.n_agents)  # 1,3
            actions_dict = [
    ...
    def values_next(
        ...
        if self.use_parameter_sharing:
            ...
            if self.use_rnn:
                ...
                if self.use_global_state:
                    critic_input = np.repeat(state.reshape([n_env, 1, -1]), self.n_agents, axis=1).reshape(
                        [batch_size, 1, -1]
                    )
                else:
                    obs_array = np.array(itemgetter(*self.agent_keys)(obs_dict))
                    critic_input = np.repeat(obs_array.reshape([n_env, 1, -1]), self.n_agents, axis=1).reshape(
                        [batch_size, 1, -1]
                    )
                agents_id = (
            ...
            rnn_hidden_critic_new, values_out = self.policy.get_values(
                observation={key: critic_input}, agent_ids=agents_id, rnn_hidden=rnn_hidden_critic_i
            )
            ...
            if self.use_rnn:
                ...
                if self.use_global_state:
                    ...
                    critic_input = dict.fromkeys(self.agent_keys, joint_obs)
            else:
                critic_input_array = np.concatenate(
                    [obs_dict[k].reshape(n_env, 1, -1) for k in self.agent_keys], axis=1
                ).reshape(n_env, -1)
                if self.use_global_state:
                    critic_input_array = state.reshape([n_env, -1])
                critic_input = dict.fromkeys(self.agent_keys, critic_input_array)
